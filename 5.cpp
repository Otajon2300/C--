...
{
    string satr;
    cout << "Iltimos, satr kiriting: ";
    getline(cin, satr);

    unordered_map<char, int> harf_sonlari;
    ...
    for (char harf : satr)
    {
        ...
        if (isalpha(harf))
        {
            ...
            char kichik_harf = tolower(harf);

            harf_sonlari[kichik_harf]++;
        }
    }

    cout << "Qatnashgan harflar soni:" << endl;
    for (const auto &pair : harf_sonlari)
    {
        cout << pair.first << ": " << pair.second << " marta" << endl;
    }
